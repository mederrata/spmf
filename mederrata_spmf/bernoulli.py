...
class BernoulliFactorization(BayesianModel):
    ...
    def decoder_function(self, x):
        ...
        Returns:
            [type]: [description]
        """
        return self.link_fn(tf.cast(x, self.dtype)*tf.cast(self.eta_i, self.dtype))
    ...
    def set_data(
        ...
        super(BernoulliFactorization, self).set_data(
    ...
    def log_likelihood_components(
        ...
        logits = theta_beta + phi
        rv_bern = tfd.Bernoulli(logits=logits)

        return rv_bern.log_prob(
...
class NormalAutoencoder(NormalFactorization):
    var_list = []
    ...
    def __init__(
        ...
        """Instantiate unconstrained dense Bernoulli autoencoder
